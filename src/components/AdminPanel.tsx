...
import { X, Eye, EyeOff, LogIn, LogOut, Clock, CheckCircle, Settings, List, Archive, Calendar, Plus, Minus } from 'lucide-react'
...
const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'menu' | 'completed' | 'create-order'>('orders')
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-2 font-semibold transition-colors ${
                            activeTab === key ? 'text-yellow-400 border-b-2 border-yellow-400' : 'text-yellow-200 hover:text-yellow-400'
                          }`}
                        >
                          {icon}
                          {label}
